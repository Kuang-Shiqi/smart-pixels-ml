"""
Physics ansatz layer: closed-form (x, y, |cot a|, SIGNED cot b) estimator from a
...
SIGN CHANGE (this version):
  cot a is still emitted UNSIGNED (|cot a| ~ 0 near normal incidence, sign is
  information-limited; the wrapper still borrows the cot a sign). cot b is now
  emitted **SIGNED**: the magnitude |cot b| comes from the cluster width (which
  is EVEN in beta and so cannot carry a sign), and the sign comes from the
  y-charge skewness (which is ODD in beta) via
        sign_b = tanh(sign_k * (skew_y - sign_g0)).
  The 2-bit teacher's borrowed cot b sign is anti-correlated with truth on
  analog inputs (~0.35 agreement), so the training loop must STOP applying it
  to the cot b slot (pass sign_b = +1 there) and let this branch own the sign.

Inputs:  charge tensor with shape (B, 16, 16, T)  with T = 2 by default
Outputs: tensor with shape (B, 4) -- order is (x, y, |cot a|, cot b[signed])
"""
...
class PhysicsAnsatz(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # ---- data-driven SIGN for cot-beta (NEW) ----
        # sign_b = tanh(sign_k * (skew_y - sign_g0)).
        #  - sign_k  : steepness; init 5.0 so it starts decisive but trainable
        #              (it can also go NEGATIVE to auto-learn the convention).
        #  - sign_g0 : skew offset; absorbs the constant skew the always-same-
        #              direction Lorentz drift bakes in, so the RESIDUAL skew
        #              sets the track sign.
        self.sign_k = self.add_weight(
            name='sign_k', shape=(),
            initializer=tf.constant_initializer(5.0),
            trainable=True,
        )
        self.sign_g0 = self.add_weight(
            name='sign_g0', shape=(),
    ...
    def call(self, charge):
        ...
        # ---- y-charge skewness -> data-driven SIGN for cot-beta (HARDENED) ----
        # NUMERICAL SAFETY: relu charge (pedestals<0), std floored at 1um^2 (was 1e-9 ->
        # std**3 underflow -> overflow/NaN), finite-guard + clip the tanh argument.
        prof_y_pos = tf.nn.relu(prof_y)
        sum_y_pos  = tf.reduce_sum(prof_y_pos, axis=1)
        ybar_pos   = _safe_div(tf.reduce_sum(prof_y_pos * y_centers, axis=1), sum_y_pos)
        diff_y     = y_centers[None, :] - ybar_pos[:, None]
        var_y      = _safe_div(tf.reduce_sum(prof_y_pos * diff_y ** 2, axis=1), sum_y_pos)
        std_y      = tf.sqrt(var_y + 1.0)
        skew_y     = _safe_div(tf.reduce_sum(prof_y_pos * diff_y ** 3, axis=1), sum_y_pos) / (std_y ** 3)
        skew_y     = tf.where(tf.math.is_finite(skew_y), skew_y, tf.zeros_like(skew_y))
        skew_y     = tf.clip_by_value(skew_y, -8.0, 8.0)
        sign_b     = tf.tanh(self.sign_k * (skew_y - self.sign_g0))   # (B,) in (-1, 1)
        ...
        # ---- cluster-width angle estimates ----
        # |cot a| = (w_x - 1) * p_x / T   (still UNSIGNED)
        ...
        # apply the learned sign -> SIGNED cot b
        ...
        if self.variant in ('localreco', 'blend'):
            ...
            # Eq 8 / Eq 9 use the UNSIGNED width-based magnitude internally.
        ...
        # NOTE slot 3 is now the SIGNED cot b.
        raw = tf.stack([x_pred, y_pred, cota_abs, cotb_signed], axis=-1)   # (B, 4)
        # Pre-divide by labels_scale so raw lives in label units; the affine
        # head then only learns small corrections instead of a huge rescale.
