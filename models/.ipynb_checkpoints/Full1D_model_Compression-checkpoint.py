...
def CreateFull1DModel(
    ...
    output_units=8,        # Output units (8 for Full model: x, y, α, β + uncertainties)
    ...
    """
    Tunable Full 1D model for smartpixel inference (8 outputs with uncertainties).
    ...
    - output_units: Should stay 8 for Full model (x, y, α, β + uncertainties)
    ...
    # ========== CONV NETWORK (same as Slim model) ==========
    ...
    # Output layer (usually not pruned) - 8 outputs for Full model
    dense_3 = Dense(output_units, name="dense_3")(activation_tanh_3)
    
    model = Model(inputs=input_layer, outputs=dense_3, name="smrtpxl_full_tunable")
    return model
